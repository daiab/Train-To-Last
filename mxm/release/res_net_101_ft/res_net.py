...
class Symble(BaseSymble):
    def def_model(self):
        ...
        .split(num_output=2, name="split")
        .fetch("data_ms_down", "data_cnfd_up"))

        (self.feed("data_ms_down")
         .softmax(name="softmax_ms"))

        (self.feed("data_cnfd_up")
         .softmax(name="softmax_cnfd"))
